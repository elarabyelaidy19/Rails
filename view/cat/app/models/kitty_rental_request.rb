class KittyRentalRequest < ApplicationRecord
  # .freeze renders constants immutable
  STATUS_STATES = %w(APPROVED DENIED PENDING).freeze

  validates :kitty_id, :end_date, :start_date, :status, presence: true
  ...
  def overlapping_requests
    ...
    # 2. That is for the same kitty.
    # 3. That overlaps.

    kittyRentalRequest
      .where.not(id: self.id)
      .where(kitty_id: kitty_id)
  ...
  def does_not_overlap_approved_request
    ...
    # periods during which a kitty has already been spoken for.
